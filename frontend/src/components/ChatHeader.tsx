...
const ChatHeader = ({ user, setSiderbarOpen, isTyping, onlineUsers }: ChatHeaderprops) => {
    const isOnlineUser = user && onlineUsers.includes(user._id)
    return <>
        <div className='sm:hidden fixed top-4 right-5 z-30 '>
            <button className='p-3 bg-gray-800 rounded-lg hover:bg-gray-700 transition-colors' onClick={() => setSiderbarOpen(true)}>
                            ...
                            <div className='flex items-center gap-2'>
                                {
                                    ...
                                    <div className='flex items-center gap-2'>
                                        ...
                                        }`}>{isOnlineUser? "Online" : "Offine"}</span>
                                    </div>
                                    )
                                }
                            </div>
